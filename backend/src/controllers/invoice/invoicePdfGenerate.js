...
async function generateInvoicePdf(data, outputStream) {

    const { customerId, consigneeId, notifyPartyId, termsOfPaymentId } = data;

    if (!customerId || !consigneeId || !notifyPartyId || !termsOfPaymentId) {
        throw new Error(`customerId, consigneeId, notifyPartyId, termsOfPaymentId can't be null`);
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        // ── Title ─────────────────────────────────────
        doc.font("Helvetica-Bold").fontSize(14).text("INVOICE", 0, OUTER_MARGIN + 6, { align: "center", width: PAGE_W });
        ...
        const r1LeftH = measureCell(doc, "Invoice No. & Date", invoiceLabel, RIGHT_COL1_W);
        ...
        cellContent(doc, "Invoice No. & Date", invoiceLabel, RIGHT_X, sub1Top, RIGHT_COL1_W);
