...
import InputBox from '../components/InputBox';
import toast from 'react-hot-toast';
import { useAddCustomerMutation } from '../customerapiSlice/apiSlicecustomer';
...
export default function AddCustomer() {
  ...
  const toggle = (key) => setOpenSection(prev => prev === key ? null : key);
  const [formData, setFormData] = useState(initialFormData);
  const [addCustomer, { isLoading: isSubmitting, error, isError, }] = useAddCustomerMutation();
  ...
  const partySummary = (arr) => hasFilled(arr) ? null : 'Click to expand and fill details';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.data?.message || err?.message || "Failed to add customer. Please try again.");
    }
  };
