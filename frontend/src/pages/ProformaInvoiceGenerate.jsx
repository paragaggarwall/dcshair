...
//                     type="button"
//                     onClick={onAdd}
//                     className="flex items-center gap-1.5 text-[10px] bg-[#003366] font-bold text-white hover:bg-[#004080] px-2.5 py-1 rounded-lg transition-all shadow-sm cursor-pointer"
//                 >
...
//         const qty = parseFloat(i.weight) || 0;
...
//         if (!d) { toast.error("Please select a contract first."); return false; }
...
//         const {
//             proformaInvoiceNo, invoiceDate, termsOfPayment, paymentterm,
//             lcnumber, lcDate, otherRef, packing, shipingMark, sizeScale,
//             countryOfOrigin, countryOfDestination, preCarriageBy, cartonweight,
...
//         try {
//             const res = await createProformaInvoice({ body: buildPayload() }).unwrap();

//             if (res?.success) {
//                 setTimeout(() => navigate("/proforma-invoice"), 1200);
//                 toast.success(res?.message || 'create successfully proforma')
//             }
//         } catch (e) {
//             toast.error(e?.data?.message ?? e?.message ?? "Failed to create proforma invoice.");
//         }
//     };
...
//                                         {["Product", "sku Code", "Size", "weight (KGS)", "Rate (US$/KGS)", "Amount (US$)"].map((h, i) => (
...
import {
    ArrowLeft, FileText, Package, Truck,
    Loader2, Download, Info, UserCheck, CheckCircle2,
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useNavigate } from "react-router-dom";
import InputBox from "../components/inputBox";
...
export default function ProformaInvoiceGenerate() {
    ...
    const fetchCustomerParties = useCallback(async (customerId) => {
        ...
        try {
            ...
            console.error(e?.data?.message || e);
            toast.error(e?.data?.message || e?.message || 'fail to fetch customer party');
        }
    }, [getCustomerbyId]);

    useEffect(() => {
        if (!d?.customerId) return;
        fetchCustomerParties(d.customerId);
    }, [d?.customerId, fetchCustomerParties]);
    ...
    const validateForm = useCallback(() => {
        const {
            proformaInvoiceNo, invoiceDate, termsOfPayment, paymentterm,
            lcnumber, lcDate, otherRef, countryOfOrigin, countryOfDestination,
            preCarriageBy, portOfLoading, portOfFinalDestination, operatingAirlines,
            flightNo, currency, consigneeId,
        } = form;
    ...
    }, [d, form, contractId, contractItems]);
    ...
    const handleCreatePartyApi = async (type, formValues) => {
        ...
        // refresh the consignee/notifyParty/contactPerson lists so the new one appears
        await fetchCustomerParties(d?.customerId);
