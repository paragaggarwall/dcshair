...
export default function InvoiceGenerate() {
    ...
    // ── RTK Query hooks ───────────────────────────────────────────────────────
    const [getprofomainvoice, { isLoading: loadingProformaList }] = useGetprofomainvoiceMutation();
    const [getprofomaparty, { isLoading: loadingPartyData }] = useGetprofomapartyMutation();
    const [getalltermofpayment, { isLoading: loadingTerms }] = useLazyGetalltermofpaymentQuery();
    const [invoicecreate, { isLoading: generatingPdf }] = useInvoicecreateMutation();
    const [getCustomerbyId, { isLoading: loadingCustomer }] = useGetCustomerbyIdMutation();
    ...
    const buildPayload = useCallback(() => {
        const {
            ...
            countryOfOrigin, countryOfDestination, preCarriageBy,
            portOfLoading, portOfFinalDestination, description,
            operatingAirlines, flightNo, sizeScale, shipingMark, cartonweight, packing,
            currency, consigneeId, notifyPartyId, contactPersonId,
        } = form;
    ...
    }, [d, form, proformaid, contractItems, totalAmount]);
    ...
    const handleGeneratePdf = async () => {
        ...
        try {
            ...
            if (res?.success) {
                ...
                toast.success(res?.message || 'create successfully invoice');
            }
